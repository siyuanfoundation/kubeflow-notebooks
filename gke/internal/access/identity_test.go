...
func TestIAPAuthentication(t *testing.T) {
	...
	cases := []struct {
		...
		{"future issuance", func(claims map[string]any) { claims["iat"] = time.Now().Unix() + 600 }, false},
		...
		{"excessive lifetime", func(claims map[string]any) { claims["exp"] = time.Now().Unix() + 90000 }, false},
